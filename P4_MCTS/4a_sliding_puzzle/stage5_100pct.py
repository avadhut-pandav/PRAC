...
if __name__ == "__main__":
    ...
    print("Start:", start, "| misplaced tiles:", heuristic(start))
    draw_state(start, "4a_puzzle_start.png", "Start State")

    state = start
    for step in range(5):
        if state == GOAL:
            break
        state = mcts(state, iterations=300)
        print(f"Step {step+1}: {state} | misplaced:", heuristic(state))
    print("Reached goal!" if state == GOAL else "Best effort reached (increase iterations for harder puzzles).")
    draw_state(state, "4a_puzzle_result.png", "Result via MCTS")
    print("Saved visualizations -> 4a_puzzle_start.png, 4a_puzzle_result.png")
