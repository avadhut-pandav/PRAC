...
if __name__ == "__main__":
    ...
    DEPTH = 4
    turn = HUMAN
    scripted_human = [3, 3, 2]
    move_no = 0
    while valid_moves(board) and not check_winner(board, HUMAN) and not check_winner(board, AI):
        if turn == HUMAN:
            col = scripted_human[move_no] if move_no < len(scripted_human) else valid_moves(board)[0]
        else:
            _, col = minimax(board, DEPTH, True)
        drop_piece(board, col, turn)
        print(f"Move {move_no+1}: {'Human' if turn==HUMAN else 'AI'} drops in column {col}")
        turn = AI if turn == HUMAN else HUMAN
        move_no += 1
        if move_no > 12:
            break

    if check_winner(board, HUMAN): print("Result: Human wins")
    elif check_winner(board, AI): print("Result: AI wins")
    else: print("Result: Game ongoing / draw (demo truncated)")
    draw_board(board, "3c_connect4_result.png")
    print("Saved visualization -> 3c_connect4_result.png")
